...
REM processor architecture check
cls
if %processor_architecture%==AMD64 (
	echo 64-bit processor detected
	echo.
	set proc=x64
	set hlex=%bin%\hllib\x64\hlextract.exe
	) else (
		echo 64-bit processor not detected
		echo defaulting to x86
		echo.
		set hlex=%bin%\hllib\x86\hlextract.exe
		set proc=x86
)

REM check to see steam directory is present (reads from variable set in settings.ini)
if exist "%cache%" (
	echo "%cache%"
	echo starting script
) else (
	echo WARNING: the steam directory you have specified does not seem to exist.
	echo please check "settings.ini" to see that you have properly set everything.
	echo.
	echo "%cache%"
	pause
	exit
...
REM check to see if steam.exe is running
tasklist /FI "IMAGENAME eq steam.exe" 2>NUL | find /I /N "steam.exe">NUL
echo.
if "%ERRORLEVEL%"=="0" (
	echo WARNING: steam.exe is running.  For best results, steam.exe should not be running so that the ripper can have exclusive access to any game cache files.
	pause
	exit
	)
...
echo  º                        Tom Steam Ripper v%ver%                          %proc%º
...
echo  ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼
echo  ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
echo  º[1]Read settings from "custom.ini"                                         º
echo  º[2]auto-detect feature                                                     º
echo  ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼
set /p ad=Make a selection: 
if /i %ad%==1 goto :cust
if /i %ad%==2 goto :autd
goto :other
...
for /f "skip=3 tokens=1 delims==" %%i in (custom.ini) do (
	...
	pause
	goto :main
	) else %hlex% -p "%cache%\%%i" -d "%back%" -e "root" -n "%cache%\common\%ncu%"
	)
	goto :oext
	
:autd
set root="%cd%"
REM cd /d "%cache%\common"
echo CURRENTLY INSTALLED GAMES in "%cache%\common"
echo ================================================================================
dir "%cache%\common" /b /ad /on
echo ================================================================================
set /p ncu=Enter the FULL NAME of the game you would like to extract: 
dir "%cache%\common" /b /ad | find /i "%ncu%">nul
if "%errorlevel%"=="1" goto :autd
set /p appid=set the appid (found on steampowered.com): 
pause
echo.
echo creating extraction index...
echo auto-detecting game files for extraction...
REM echo filer>index.tmp
...
REM pause
REM exit
for /f "tokens=1 delims==" %%i in (index.tmp) do (
	echo extracting %%i
	if not exist "%cache%\%%i" (
	echo %%i does not exist in "%cache%"
	echo Operation was interrupted!
...
for /f "eol=# skip=1 tokens=1 delims==" %%i in (%bin%\apps\%sel%.ini) do (
	...
	goto :main
			...
			md "%back%\platform"
			%hlex% -p "%cache%\%%i" -d "%back%\platform" -e "root"
			move "%back%\platform\root" "%back%\root\platform"
			rd "%back%\platform"
			) else if not %%i==platform.gcf %hlex% -p "%cache%\%%i" -d "%back%" -e "root"
	)
ren "%back%\root" %sel%
...
for /f "eol=# skip=1 tokens=1 delims==" %%i in (%bin%\apps\%sel%.ini) do (
	...
	goto :main
		...
		%hlex% -p "%cache%\%%i" -d "%back%" -e "root" -n "%cache%\common\left 4 dead"
	) else if /i %sel%==l4d2 (
		%hlex% -p "%cache%\%%i" -d "%back%" -e "root" -n "%cache%\common\left 4 dead 2"
	) else if /i %sel%==ptl2 (
		%hlex% -p "%cache%\%%i" -d "%back%" -e "root" -n "%cache%\common\portal 2"
	) else %hlex% -p "%cache%\%%i" -d "%back%" -e "root"
	)
ping localhost -n 2 >nul
ren "%back%\root" %sel%
ping localhost -n 2 >nul
...
if /i %sel%==tf2 (
	...
	copy /y %bin%\revloader\revLoader.exe "%back%\%sel%"
	type "%bin%\revloader\rev.ini" | find /v "ProcName">"%back%\%sel%\rev.ini"
	echo ProcName=portal2.exe>>"%back%\%sel%\rev.ini"
	echo 620>"%back%\%sel%\steam_appid.txt"
)
pause
set /p ad=Would you like to install any addons? [y/N]
if /i %ad%==y goto :addons
...
:oext
cls
REM for /f "eol=# tokens=1,2 delims==" %%i in (custom.ini) do (
	REM set %%i=%%j
	REM )
REM if not exist "%cache%\common\%ncu%" (
	REM echo "%cache%\common\%ncu%" does not exist
	REM pause
	REM goto :main
	REM )
